...
void ColtRun(const char* byte_code_out, StringView to_parse)
{
	...
	//The flags for the byte code generator
	ByteCodeGeneratorFlags gen_flags;
	//Choose default options
	memset(&gen_flags, 0, sizeof(ByteCodeGeneratorFlags));

	if (ASTParse(&ast, to_parse, &options))
	{
		Chunk chunk = generateByteCode(&ast.table, &ast.expr, &gen_flags);
...
void ColtREPL(const char* byte_code_out)
{
	...
	//The flags for the byte code generator
	ByteCodeGeneratorFlags gen_flags = { .no_generate_debug_symbols = true, .print_last_expr = true };
	...
	while (!feof(stdin))
	{
		...
		if (line.size != 1) //if string not empty
		{
			...
			repl_run(&ast, StringToStringViewWithNUL(&line), &options, &gen_flags, byte_code_out);
		}
		StringFree(&line);
	}
	ASTFree(&ast);
...
void repl_run(AST* ast, StringView strv, const ColtScanOptions* options, const ByteCodeGeneratorFlags* flags, const char* byte_code_out)
{
	...
	{
		Chunk chunk = generateByteCode(&ast->table, &ast->expr, flags);
		if (byte_code_out != NULL)
			ChunkSerialize(&chunk, byte_code_out);
