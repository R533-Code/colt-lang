...
DWORD ChunkGetDWORD(const Chunk* chunk, uint64_t* offset)
{
	...
	DWORD return_val = { .u32 = *(int32_t*)(chunk->code + local_offset) };
	//Update the value of the offset
...
QWORD ChunkGetQWORD(const Chunk* chunk, uint64_t* offset)
{
	...
	QWORD return_val = { .u64 = *(int64_t*)(chunk->code + local_offset) };
	//Update the value of the offset
...
{
	chunk_grow_size(chunk, more_byte_capacity);
}
...
Chunk ChunkDeserialize(const char* path)
{
	...
	Chunk chunk = { .code = safe_malloc(file_size), .capacity = file_size, .count = file_size };
...
WORD unsafe_get_word(uint8_t** ptr)
{
	*ptr += (2 - ((uint64_t)(*ptr) & 1)); //read past padding
	WORD return_val = { .u16 = *((uint16_t*)*ptr) };
	*ptr += sizeof(int16_t);
	return return_val;
}
...
QWORD unsafe_get_qword(uint8_t** ptr)
{
	*ptr += (8 - ((uint64_t)(*ptr) & 7)); //read past padding
	QWORD return_val = { .u64 = *((uint64_t*)*ptr) };
	*ptr += sizeof(int64_t);
	return return_val;
}
