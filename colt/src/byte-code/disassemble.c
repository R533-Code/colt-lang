...
uint64_t impl_chunk_print_code(const Chunk* chunk, uint64_t offset)
{
	...
	case OP_PUSH_SCOPE:
		impl_print_hex_instruction("OP_PUSH_SCOPE", ChunkGetDWORD(chunk, &offset).u32);
		return offset;
	
	case OP_POP_SCOPE:
		impl_print_hex_instruction("OP_POP_SCOPE", ChunkGetDWORD(chunk, &offset).u32);
		return offset;

	case OP_SLOAD_LOCAL:
		impl_print_hex_instruction("OP_SLOAD_LOCAL", ChunkGetBYTE(chunk, &offset).u8);
		return offset;

	case OP_SSTORE_LOCAL:
		impl_print_hex_instruction("OP_SSTORE_LOCAL", ChunkGetBYTE(chunk, &offset).u8);
		return offset;

	case OP_LOAD_LOCAL:
		impl_print_hex_instruction("OP_LOAD_LOCAL", ChunkGetDWORD(chunk, &offset).u32);
		return offset;

	case OP_STORE_LOCAL:
		impl_print_hex_instruction("OP_STORE_LOCAL", ChunkGetDWORD(chunk, &offset).u32);
		return offset;
	...
	case OP_BOOL_NOT:
		return impl_print_operand_instruction("OP_BOOL_NOT", chunk->code[offset + 1], offset);
	case OP_CMP_GREATER:
