...
namespace colti::args
{
	ParseResult ParseArguments(int argc, const char** argv)
	{
		...
		for (size_t i = 1; i < argc; i++)
		{
			if (argv[i][0] == '-')
			{
				CmdArgument arg = impl_string_to_arg(argv[i]);
				switch (arg)
				{
...
namespace colti::args::details
{
	CmdArgument impl_string_to_arg(const char* str)
	{
	...
	void impl_help(int argc, const char** argv, uint64_t offset)
	{
		...
		{
			CmdArgument arg = impl_string_to_arg(argv[2]);
			switch (arg)
			{
			break; case ARG_HELP:
				impl_help_help();
